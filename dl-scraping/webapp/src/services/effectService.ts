...
class EffectService {
  ...
  downloadImage(effects: EffectInfo[]): void {
    try {
      effects.map(async (effect) => {
        const response = await fetch("https://asia-northeast1-asa-o-experiment.cloudfunctions.net/get-effect-image", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  async change(hdshId: string) {
    ...
    const response = await fetch("https://asia-northeast1-asa-o-experiment.cloudfunctions.net/change-effect", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
